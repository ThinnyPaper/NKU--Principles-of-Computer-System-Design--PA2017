...
static inline void idex(vaddr_t *eip, opcode_entry *e) {
  ...
  if (e->decode)
    e->decode(eip); //decode is a function. decode_id will call the make_DHelper(id) in decode.c
  e->execute(eip);  //execute exec_ex will call the function make_EHelper(ex)
}
...
/* 0x80, 0x81, 0x83 */
//opcode_entry opcode_table_gp1[8]{empty,empty...}
//static make_EHelper(gp1){ idex(eip,&opcode_table_gp1[decoding.ext_opcode]);}
make_group(gp1,
